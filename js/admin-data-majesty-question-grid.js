...
EditableGrid.prototype.initializeGrid = function() 
{
	with (this) {
		...
		setHeaderRenderer("subject", new InfoHeaderRenderer("Mata Pelajaran"));
		setHeaderRenderer("question", new InfoHeaderRenderer("Pertanyaan"));
		setHeaderRenderer("answer", new InfoHeaderRenderer("Jawaban"));
		setHeaderRenderer("option_a", new InfoHeaderRenderer("Opsi A"));
		setHeaderRenderer("option_b", new InfoHeaderRenderer("Opsi B"));
		setHeaderRenderer("option_c", new InfoHeaderRenderer("Opsi C"));
		setHeaderRenderer("option_d", new InfoHeaderRenderer("Opsi D"));
		setHeaderRenderer("option_e", new InfoHeaderRenderer("Opsi E"));
		setHeaderRenderer("book", new InfoHeaderRenderer("Nama Buku"));
		setHeaderRenderer("video", new InfoHeaderRenderer("Nama Video"));
		...
		setCellRenderer("action", new CellRenderer({render: function(cell, value) {
			...
			cell.innerHTML = "<a href='admin-update-data-majesty-question.php?update=" + rowId + "'\" style=\"cursor:pointer\">" +
			"<img src=\"" + image("edit.png") + "\" width=\"16px\" height=\"16px\" border=\"0\" alt=\"ubah\" title=\"Ubah\"/></a>";

			cell.innerHTML += "&nbsp;<a onclick=\"if (confirm('Apakah anda akan menghapus data tersebut? ')) { href='query/admin-remove-data-majesty-question.php?remove=" + rowId + "' } \" style=\"cursor:pointer\">" +
...
EditableGrid.prototype.onloadHTML = function(tableId) 
{
	...
	this.load({ metadata: [
	                       ...
	                       { name: "answer", datatype: "html", editable: false },
	                       { name: "option_a", datatype: "html", editable: false },
	                       { name: "option_b", datatype: "html", editable: false },
	                       { name: "option_c", datatype: "html", editable: false },
	                       { name: "option_d", datatype: "html", editable: false },
	                       { name: "option_e", datatype: "html", editable: false },
	                       { name: "book", datatype: "html", editable: false },
	                       { name: "video", datatype: "html", editable: false },
...
{
	//this.renderBarChart("barchartcontent", 'Data Soal STAN dan Kedinasan' + (this.getRowCount() <= this.maxBars ? '' : ' (' + this.maxBars + ' baris dari ' + this.getRowCount() + ' baris)'), 'subject', { limit: this.maxBars, bar3d: false, rotateXLabels: this.maxBars > 10 ? 270 : 0 });
	this.renderPieChart("piechartcontent", 'Data Soal STAN dan Kedinasan', 'subject', 'subject');
};

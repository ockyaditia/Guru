...
EditableGrid.prototype.initializeGrid = function() 
{
	with (this) {
		...
		setHeaderRenderer("class", new InfoHeaderRenderer("Kelas"));
		setHeaderRenderer("age", new InfoHeaderRenderer("Umur"));
		...
		setCellRenderer("action", new CellRenderer({render: function(cell, value) {
			...
			cell.innerHTML = "<a href='admin-update-data-learning-level.php?update=" + rowId + "'\" style=\"cursor:pointer\">" +
			"<img src=\"" + image("edit.png") + "\" width=\"16px\" height=\"16px\" border=\"0\" alt=\"ubah\" title=\"Ubah\"/></a>";

			cell.innerHTML += "&nbsp;<a onclick=\"if (confirm('Apakah anda akan menghapus data tersebut? ')) { href='query/admin-remove-data-learning-level.php?remove=" + rowId + "' } \" style=\"cursor:pointer\">" +
...
EditableGrid.prototype.onloadHTML = function(tableId) 
{
	...
	this.load({ metadata: [
	                       ...
	                       { name: "age", datatype: "html", editable: false },
...
{
	//this.renderBarChart("barchartcontent", 'Data Jenjang Pembelajaran' + (this.getRowCount() <= this.maxBars ? '' : ' (' + this.maxBars + ' baris dari ' + this.getRowCount() + ' baris)'), 'name', { limit: this.maxBars, bar3d: false, rotateXLabels: this.maxBars > 10 ? 270 : 0 });
	this.renderPieChart("piechartcontent", 'Data Jenjang Pembelajaran', 'name', 'name');
};

...
EditableGrid.prototype.initializeGrid = function() 
{
	with (this) {
		...
		setCellRenderer("action", new CellRenderer({render: function(cell, value) {
			...
			cell.innerHTML = "<a href='admin-update-data-tkd.php?update=" + rowId + "'\" style=\"cursor:pointer\">" +
			"<img src=\"" + image("edit.png") + "\" width=\"16px\" height=\"16px\" border=\"0\" alt=\"ubah\" title=\"Ubah\"/></a>";

			cell.innerHTML += "&nbsp;<a onclick=\"if (confirm('Apakah anda akan menghapus data tersebut? ')) { href='query/admin-remove-data-tkd.php?remove=" + rowId + "' } \" style=\"cursor:pointer\">" +
...
{
	//this.renderBarChart("barchartcontent", 'Data Tes Kompetensi Dasar (TKD)' + (this.getRowCount() <= this.maxBars ? '' : ' (' + this.maxBars + ' baris dari ' + this.getRowCount() + ' baris)'), 'name', { limit: this.maxBars, bar3d: false, rotateXLabels: this.maxBars > 10 ? 270 : 0 });
	this.renderPieChart("piechartcontent", 'Data Tes Kompetensi Dasar (TKD)', 'name', 'name');
};

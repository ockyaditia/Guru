...
EditableGrid.prototype.initializeGrid = function() 
{
	with (this) {
		...
		setHeaderRenderer("user_code", new InfoHeaderRenderer("Nama Bank"));
		setHeaderRenderer("package_code", new InfoHeaderRenderer("Nama Bank"));
		setHeaderRenderer("payment_code", new InfoHeaderRenderer("Nama Bank"));
		setHeaderRenderer("bank_name", new InfoHeaderRenderer("Nama Bank"));
		setHeaderRenderer("account_name", new InfoHeaderRenderer("Nama Rekening"));
		setHeaderRenderer("account_number", new InfoHeaderRenderer("Nomor Rekening"));
		setHeaderRenderer("img", new InfoHeaderRenderer("Gambar"));
		setHeaderRenderer("approval", new InfoHeaderRenderer("Persetujuan"));
		...
		setCellRenderer("action", new CellRenderer({render: function(cell, value) {
			...
			cell.innerHTML += "&nbsp;<a onclick=\"if (confirm('Apakah anda akan menghapus data tersebut? ')) { href='query/admin-remove-data-transactions.php?remove=" + rowId + "' } \" style=\"cursor:pointer\">" +
...
EditableGrid.prototype.onloadHTML = function(tableId) 
{
	...
	this.load({ metadata: [
	                       { name: "user_code", datatype: "html", editable: false },
	                       { name: "package_code", datatype: "html", editable: false },
	                       { name: "payment_code", datatype: "html", editable: false },
	                       { name: "bank_name", datatype: "html", editable: false },
	                       { name: "account_name", datatype: "html", editable: false },
	                       { name: "account_number", datatype: "html", editable: false },
	                       { name: "img", datatype: "html", editable: false },
	                       { name: "approval", datatype: "html", editable: false },
...
{
	//this.renderBarChart("barchartcontent", 'Data Transaksi' + (this.getRowCount() <= this.maxBars ? '' : ' (' + this.maxBars + ' baris dari ' + this.getRowCount() + ' baris)'), 'user_code', { limit: this.maxBars, bar3d: false, rotateXLabels: this.maxBars > 10 ? 270 : 0 });
	this.renderPieChart("piechartcontent", 'Data Transaksi', 'user_code', 'user_code');
};

...
EditableGrid.prototype.initializeGrid = function() 
{
	with (this) {
		...
		setHeaderRenderer("email", new InfoHeaderRenderer("Email"));
		setHeaderRenderer("name", new InfoHeaderRenderer("Nama"));
		setHeaderRenderer("status", new InfoHeaderRenderer("Status"));
		setHeaderRenderer("phone_number", new InfoHeaderRenderer("Nomor Telepon"));
		setHeaderRenderer("facebook", new InfoHeaderRenderer("Facebook"));
		setHeaderRenderer("twitter", new InfoHeaderRenderer("Twitter"));
		setHeaderRenderer("instagram", new InfoHeaderRenderer("Instagram"));
		...
		setCellRenderer("action", new CellRenderer({render: function(cell, value) {
			...
			cell.innerHTML = "<a href='admin-update-data-user.php?update=" + rowId + "'\" style=\"cursor:pointer\">" +
			"<img src=\"" + image("edit.png") + "\" width=\"16px\" height=\"16px\" border=\"0\" alt=\"ubah\" title=\"Ubah\"/></a>";

			cell.innerHTML += "&nbsp;<a onclick=\"if (confirm('Apakah anda akan menghapus data tersebut? ')) { href='query/admin-remove-data-user.php?remove=" + rowId + "' } \" style=\"cursor:pointer\">" +
...
EditableGrid.prototype.onloadHTML = function(tableId) 
{
	...
	this.load({ metadata: [
	                       { name: "email", datatype: "html", editable: false },
	                       { name: "name", datatype: "html", editable: false },
	                       { name: "status", datatype: "html", editable: false },
	                       { name: "phone_number", datatype: "html", editable: false },
	                       { name: "facebook", datatype: "html", editable: false },
	                       { name: "twitter", datatype: "html", editable: false },
	                       { name: "instagram", datatype: "html", editable: false },
...
{
	//this.renderBarChart("barchartcontent", 'Data Pengguna' + (this.getRowCount() <= this.maxBars ? '' : ' (' + this.maxBars + ' baris dari ' + this.getRowCount() + ' baris)'), 'status', { limit: this.maxBars, bar3d: false, rotateXLabels: this.maxBars > 10 ? 270 : 0 });
	this.renderPieChart("piechartcontent", 'Data Pengguna', 'status', 'status');
};

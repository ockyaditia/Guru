...
EditableGrid.prototype.initializeGrid = function() 
{
	with (this) {
		...
		setHeaderRenderer("package", new InfoHeaderRenderer("Jumlah Soal"));
		...
		setCellRenderer("action", new CellRenderer({render: function(cell, value) {
			...
			cell.innerHTML = "<a href='admin-update-data-majesty-package-detail.php?update=" + rowId + "'\" style=\"cursor:pointer\">" +
			"<img src=\"" + image("edit.png") + "\" width=\"16px\" height=\"16px\" border=\"0\" alt=\"ubah\" title=\"Ubah\"/></a>";

			cell.innerHTML += "&nbsp;<a onclick=\"if (confirm('Apakah anda akan menghapus data tersebut? ')) { href='query/admin-remove-data-majesty-package-detail.php?remove=" + rowId + "' } \" style=\"cursor:pointer\">" +
...
{
	//this.renderBarChart("barchartcontent", 'Data Paket' + (this.getRowCount() <= this.maxBars ? '' : ' (' + this.maxBars + ' baris dari ' + this.getRowCount() + ' baris)'), 'package', { limit: this.maxBars, bar3d: false, rotateXLabels: this.maxBars > 10 ? 270 : 0 });
	this.renderPieChart("piechartcontent", 'Data Paket', 'package', 'package');
};

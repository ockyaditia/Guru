...
<?php
	...
	if (isset($_GET['code'])) {
		...
		if ($count == 0) {
			header('location:payment-majesty-package.php?package_code='.$package_code);
		} else {
			if ($approval != 3) {
				header('location:transactions-majesty-package.php?package_code='.$package_code);
			}
		} 
	}
?>
...
<body>
	...
	<?php
		...
		if (isset($_GET['code'])) {
			...
			$sql_data = "SELECT * FROM majesty_package_list WHERE package='$package_code'";
								...
								var questions = [
									<?php
										while ($data = $result_data->fetch_assoc()) {
											...
											$sql_row = "SELECT * FROM majesty_question WHERE code='$question'";

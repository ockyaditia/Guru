<?php
	...
	$location = "location:../transactions-majesty-package.php?fail-message=";
	...
	if (!$result = $mysqli->query($sql)) {
		...
		header('location:../transactions-majesty-package.php?package_code='.$package_code.'&fail='.$mysqli->errno);
	} else {
		if (!move_uploaded_file($_FILES["img"]["tmp_name"], $target_dir . $newfilename)) {
			$error = "Ada sesuatu yang gagal dalam unduh gambar.";
			header($location.$error);
		}
		header('location:../transactions-majesty-package.php?package_code='.$package_code.'&success=1');
	}
?>